...
export default function App() {
  ...
  useEffect(() => {
    ...
    let i = 0;
    const t = setInterval(() => { i++; setVisible(i); if (i >= initMsgs.length) clearInterval(t); }, 380);
        ...
        <header style={{ background: H.white, borderBottom: `1px solid ${H.border}`, padding: "12px 16px 0", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 10 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
              {["记忆", "设置"].map(t => (<button key={t} style={{ fontSize: 10.5, color: H.honeyDeep, background: H.honeyLight, border: `1px solid ${H.honeyMid}`, borderRadius: 999, padding: "3px 10px", cursor: "pointer", letterSpacing: ".08em" }}>{t}</button>))}
            </div>
          </div>
          <Stars />
        </header>
