...
function normalizedRun(run) {
  ...
  const userColor = state.user_color === 'white' ? 'white' : 'black';
  const luzeColor = state.luze_color === 'black' ? 'black' : 'white';
  const rawMoves = Array.isArray(state.moves) ? state.moves : [];
  const moves = sanitizeMoves(rawMoves, userColor);
  const terminal = terminalResult(moves);
  const completed = run?.status === 'completed' || Boolean(terminal);
  const turn = completed ? 'done' : nextTurn(moves, userColor);
  const derivedResult = terminal ? {
  ...
  const storedMoveCount = Number(state.move_count);
  const repaired = rawMoves.length !== moves.length
    || (state.turn && state.turn !== turn)
    || (Number.isFinite(storedMoveCount) && storedMoveCount !== moves.length)
    || (run?.status === 'active' && completed);
...
function GomokuGame({ initialRun, onClose, onRefresh }) {
  ...
  const luzeTurnLockRef = useRef(false);
  ...
  const installRun = useCallback(run => {
    if (!run?.id) return;
    const safe = normalizedRun(run);
    installedRunRef.current = safe.id;
    luzeTurnLockRef.current = false;
    setRunId(safe.id);
    setMoves(safe.moves);
    setTurn(safe.turn);
    setUserColor(safe.userColor);
    setLuzeColor(safe.luzeColor);
    setStatus(safe.status);
    setResult(safe.result);
    setError('');

    const notices = [];
    if (isLocalRunId(safe.id)) notices.push('云端记录暂时没接上，这盘先在本机下，不影响对弈。');
    if (safe.repaired) notices.push('检测到旧棋谱里有连续落子或未收尾状态，已经自动退回最后一个合法回合。');
    setLocalNotice(notices.join(' '));

    if (safe.repaired && !isLocalRunId(safe.id)) {
      const patch = {
        status: safe.status,
        state: runState(safe.moves, safe.turn, safe.userColor, safe.luzeColor),
        result: safe.result,
      };
      patchJson(`/toybox/runs/${safe.id}`, patch)
        .then(() => onRefresh?.())
        .catch(err => console.warn('Gomoku cloud repair deferred:', err.message));
    }
  }, [onRefresh]);
  ...
  const persistMove = useCallback(async (actor, move, nextMoves, nextTurn) => {
    ...
    // State is the source of truth. Save it first so a decorative event-log
    // failure can never freeze the actual game.
    ...
    }).catch(err => console.warn('Gomoku event log skipped:', err.message));
  ...
  const placeUser = async (row, col) => {
    ...
    try {
      ...
      // The local board remains playable. The next successful state save contains
      // the whole board and therefore heals a missed cloud patch.
      setError(`${err.message || '这一手没有保存好'}。棋盘先保留，下一手会继续尝试同步。`);
      setTurn('luze');
    } finally {
      setBusy(false);
    }
  ...
  useEffect(() => {
    ...
    if (luzeTurnLockRef.current) return undefined;

    luzeTurnLockRef.current = true;
    setBusy(true);
    const snapshot = moves;
    const timer = window.setTimeout(async () => {
      const cell = chooseLuzeMove(snapshot);
      if (!cell) {
        luzeTurnLockRef.current = false;
        setBusy(false);
        setTurn('done');
        await completeRun('draw', snapshot, 'board_full').catch(err => setError(err.message || '平局记录没有收好'));
        return;
      }

      const move = { ...cell, actor: 'luze' };
      const nextMoves = [...snapshot, move];
      const nextBoard = boardFromMoves(nextMoves);
      const wins = isWinningMove(nextBoard, move.row, move.col, 'luze');
      const full = nextMoves.length >= BOARD_SIZE * BOARD_SIZE;
      const next = wins || full ? 'done' : 'user';

      // Critical ordering: close Luze's turn in local state before the board
      // mutation can trigger another render/effect. The ref is a second guard.
      setTurn(next);
      setMoves(nextMoves);

      let completionPromise = null;
      if (wins) completionPromise = completeRun('luze', nextMoves);
      else if (full) completionPromise = completeRun('draw', nextMoves, 'board_full');

      try {
        await persistMove('luze', move, nextMoves, next);
        if (completionPromise) await completionPromise;
      } catch (err) {
        setError(`${err.message || '陆泽这一步没有保存好'}。棋盘已经停在这一手，不会继续自己下。`);
        if (completionPromise) await completionPromise.catch(() => null);
      } finally {
        luzeTurnLockRef.current = false;
        setBusy(false);
      }
  ...
  const undo = async () => {
    ...
    try {
      if (localOnly) {
        ...
        await patchJson(`/toybox/runs/${runId}`, { state: runState(nextMoves, 'user', userColor, luzeColor) });
        postJson(`/toybox/runs/${runId}/events`, {
          actor: 'user',
          event_type: '悔棋',
          payload: { title: '你悄悄撤回了一轮 👿' },
        }).catch(() => null);
      }
      setTurn('user');
    } catch (err) {
  ...
  return (
    <section className="gomoku-overlay" aria-label="五子棋">
      ...
      <p className="gomoku-footnote">棋力始终在本地计算，不会每落一子调用模型。每回合最多只允许陆泽落一颗；旧棋谱若出现连续落子，也会自动退回最后一个合法回合。</p>
    </section>
  );
}
...
export default function ToyBoxGomokuIntegrationV2() {
  ...
  useEffect(() => {
    refreshOpen();
    const timer = window.setInterval(refreshOpen, 6000);
