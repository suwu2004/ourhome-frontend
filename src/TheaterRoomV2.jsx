...
export function TheaterRoom({ visible, theme, leaveRoom, selectedModel, availableModels = [], mainChatBackground = {} }) {
  ...
  const modelControl = ({ settings = false } = {}) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: settings ? 10 : 0 }}>
      ...
      <button type="button" onClick={refreshModels} disabled={modelsLoading} aria-label="重新拉取当前 API 站点的模型" title={modelsError || '重新拉取当前 API 站点的模型'} style={{ width: settings ? 30 : 26, height: settings ? 30 : 26, flexShrink: 0, borderRadius: '50%', border: `1px solid ${modelsError ? C.blushDeep : C.border}`, background: C.surface, color: modelsError ? C.blushDeep : C.honeyDeep, cursor: modelsLoading ? 'default' : 'pointer', fontFamily: 'inherit', opacity: modelsLoading ? .55 : 1 }}>{modelsLoading ? '…' : '↻'}</button>
    </div>
  );
  ...
  const renderChat = () => <main style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', padding: '10px min(16px, 4vw) 14px' }}>
    <div style={{ flexShrink: 0, maxWidth: 760, width: '100%', margin: '0 auto 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}><div style={{ minWidth: 0, color: C.mutedLight, fontSize: 10.5, letterSpacing: '.06em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{mode === 'interactive' ? '互动推进' : '沉浸纯文'} · 最低 {bookDraft.settings.min_reply_chars} 字 · {model || '默认模型'}</div><button type="button" onClick={() => setBookPane('settings')} style={{ border: `1px solid ${C.border}`, borderRadius: 999, background: C.surface, color: C.honeyDeep, padding: '7px 12px', fontFamily: 'inherit' }}>设定</button></div>
    <div style={{ flex: 1, minHeight: 0, maxWidth: 760, width: '100%', margin: '0 auto', border: `1px solid ${C.border}`, borderRadius: 18, ...theaterChatBackgroundStyle(), padding: 12, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, border: `1.5px solid ${editingMessage ? C.honey : C.border}`, borderRadius: 22, background: C.surface, padding: '6px 6px 6px 10px' }}><textarea value={chatInput} onChange={event => setChatInput(event.target.value)} rows={1} placeholder={editingMessage ? '修改好后重新发送…' : '在小剧场里说话、下导演指令、继续剧情……'} style={{ flex: 1, maxHeight: 120, border: 0, outline: 0, background: 'transparent', color: C.text, resize: 'none', fontFamily: 'inherit', fontSize: 14.5, lineHeight: 1.5, padding: '6px 0' }} /><button type="button" onClick={() => sendChat()} disabled={chatting || messageActionLoading || !chatInput.trim()} aria-label={editingMessage ? '重新发送修改后的小剧场消息' : '发送小剧场消息'} style={{ width: 36, height: 36, border: 0, borderRadius: '50%', background: chatInput.trim() && !chatting && !messageActionLoading ? `linear-gradient(150deg, ${C.honey}, ${C.honeyDeep})` : C.honeyMid, color: C.white, fontFamily: 'inherit', opacity: chatting || messageActionLoading ? .62 : 1 }}>↑</button></div>
  ...
  return <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', opacity: visible ? 1 : 0, pointerEvents: visible ? 'auto' : 'none', transition: 'opacity .4s ease', background: C.cream }}>
    <header className="ourhome-safe-top" style={{ background: C.white, borderBottom: `1px solid ${C.border}`, paddingLeft: 16, paddingRight: 16, paddingBottom: 12, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10 }}><span onClick={selectedBook ? goBackToShelf : leaveRoom} style={{ fontSize: 18, color: C.honeyDeep, cursor: 'pointer', padding: 4 }}>←</span><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 16, fontWeight: 700, color: C.text, letterSpacing: '.05em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selectedBook ? selectedBook.title : '小剧场'}</div><div style={{ fontSize: 10, color: C.mutedLight, letterSpacing: '.08em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selectedBook ? (bookPane === 'chat' ? `${assistantDisplayName} · ${model || '默认模型'}` : '小剧场设定') : 'theater bookshelf'}</div></div><button type="button" onClick={loadBooks} disabled={loadingBooks} aria-label="刷新小剧场书架和消息" title="刷新小剧场书架和消息" style={{ width: 32, height: 32, border: `1px solid ${C.border}`, background: C.surface, color: C.honeyDeep, borderRadius: '50%', fontFamily: 'inherit', fontSize: 15, opacity: loadingBooks ? .55 : 1 }}>{loadingBooks ? '…' : '↻'}</button></header>
